"""Voice audit with Supabase persistence."""
...
def member_joined_at(member: discord.Member) -> dt.datetime:
    return parse_time(member.joined_at or now()) or now()
...
def load_activity(guild_id: int) -> dict[int, dict]:
    with db() as conn, conn.cursor() as cur:
        cur.execute("select * from public.discordbot_member_activity where guild_id=%s", (guild_id,))
...
async def db_error(interaction: discord.Interaction, exc: Exception) -> None:
    logger.exception("voice audit task failed", exc_info=exc)
    message = f"❌ 음성 관리 작업 실패\n오류: `{type(exc).__name__}`"
    if interaction.response.is_done():
...
class ReportChannelSelect(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if not isinstance(channel, discord.TextChannel):
            await interaction.response.send_message("텍스트 채널만 설정할 수 있습니다.", ephemeral=True)
            return
        await asyncio.to_thread(self.cog.set_report_channel_id, interaction.guild.id, channel.id)
...
class VoiceAuditView(discord.ui.View):
    def __init__(self, cog: "VoiceAuditCog"):
        ...
        self.cog = cog
        self.add_item(ReportChannelSelect(cog))
...
class VoiceAuditCog(commands.Cog):
    ...
    def reconcile_voice_state(self, guild: discord.Guild) -> None:
        ...
        with db() as conn, conn.cursor() as cur:
            cur.execute("select * from public.discordbot_active_voice_sessions where guild_id=%s", (guild.id,))
    ...
    def check_warnings(self, guild: discord.Guild, members: list[discord.Member], max_per_member: int = 1) -> list[dict]:
        ...
        with db() as conn, conn.cursor() as cur:
            ...
            if not cur.fetchone()["locked"]:
                return []
            cur.execute("select * from public.discordbot_member_activity where guild_id=%s for update", (guild.id,))
            for row in cur.fetchall():
                ...
                count = min(math.floor((current - reference).total_seconds() / (WARNING_DAYS * 86400)), max_per_member)
                if count <= 0:
                    continue
    ...
    def build_report_embeds(self, guild: discord.Guild, week: int, awarded: list[dict]) -> list[discord.Embed]:
        ...
        if not awarded:
            ...
            embed.set_footer(text=f"기준: 가입 7일 이상 + 음성채널 7일 이상 미접속 · {report_date} KST")
            return [embed]
        lines = [
        ...
        for page, chunk_lines in enumerate(chunks, start=1):
            ...
            embed.add_field(name="기준", value="가입 7일 이상 + 음성채널 7일 이상 미접속", inline=True)
    ...
    async def send_weekly_report(self, guild: discord.Guild, report_key: str, week: int) -> None:
        ...
        for embed in self.build_report_embeds(guild, week, awarded):
