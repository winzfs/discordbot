"""Components V2 기반 고정 모집 패널과 공개 모집 게시물."""
from __future__ import annotations

import logging

import discord
from discord.ext import commands

from .render import (
    ACCENT_COLOR,
    compact_party_text,
    member_count,
    member_mentions,
    mode_emoji,
    party_link,
    progress_bar,
    role_emoji,
    status_info,
)
from .store import RecruitState, RecruitStore

logger = logging.getLogger(__name__)
DEFAULT_EXPIRE_SECONDS = 90 * 60


def _v2_edit_kwargs(view: discord.ui.LayoutView) -> dict[str, object]:
    """기존 content/embed 메시지를 V2 레이아웃으로 안전하게 교체한다."""
    return {
        "content": None,
        "embed": None,
        "attachments": [],
        "view": view,
    }


async def refresh_panel(bot: commands.Bot, store: RecruitStore, guild_id: int) -> None:
    """서버의 고정 모집 허브를 최신 상태로 갱신한다."""
    config = store.get_panel(guild_id)
    if not config:
        return

    channel = bot.get_channel(int(config.get("channel_id", 0)))
    if not isinstance(channel, discord.TextChannel):
        return

    view = RecruitPanelView(bot, store, guild_id)
    try:
        message = await channel.fetch_message(int(config.get("panel_message_id", 0)))
        await message.edit(**_v2_edit_kwargs(view))
    except (discord.NotFound, discord.Forbidden):
        try:
            message = await channel.send(view=view)
        except discord.HTTPException:
            logger.exception("모집 패널 재생성 실패: guild=%s", guild_id)
            return
        store.set_panel(guild_id, channel.id, message.id)
    except discord.HTTPException:
        logger.exception("모집 패널 갱신 실패: guild=%s", guild_id)


async def refresh_recruit_post(
    bot: commands.Bot,
    store: RecruitStore,
    state: RecruitState,
) -> bool:
    """개별 공개 모집 글을 상태 저장소 기준으로 갱신한다."""
    channel = bot.get_channel(int(state.get("channel_id", 0)))
    if not isinstance(channel, discord.TextChannel):
        return False
    try:
        message = await channel.fetch_message(int(state.get("message_id", 0)))
        await message.edit(**_v2_edit_kwargs(RecruitPostView(bot, store, state)))
        return True
    except (discord.NotFound, discord.Forbidden, discord.HTTPException):
        logger.warning("모집 글 갱신 실패: message=%s", state.get("message_id"))
        return False


async def delete_recruit_post(bot: commands.Bot, state: RecruitState) -> None:
    """공개 모집 글을 가능한 경우 삭제한다."""
    channel = bot.get_channel(int(state.get("channel_id", 0)))
    if not isinstance(channel, discord.TextChannel):
        return
    try:
        message = await channel.fetch_message(int(state.get("message_id", 0)))
        await message.delete()
    except (discord.NotFound, discord.Forbidden, discord.HTTPException):
        pass


class _PanelCreateButton(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            label="새 파티 만들기",
            emoji="➕",
            style=discord.ButtonStyle.success,
            custom_id="recruit:panel:create",
        )

    async def callback(self, interaction: discord.Interaction) -> None:
        view = self.view
        if isinstance(view, RecruitPanelView):
            await view.open_builder(interaction)


class _PanelMyPartyButton(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            label="내 파티",
            emoji="👤",
            style=discord.ButtonStyle.primary,
            custom_id="recruit:panel:mine",
        )

    async def callback(self, interaction: discord.Interaction) -> None:
        view = self.view
        if isinstance(view, RecruitPanelView):
            await view.open_dashboard(interaction)


class _PanelRefreshButton(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            label="새로고침",
            emoji="🔄",
            style=discord.ButtonStyle.secondary,
            custom_id="recruit:panel:refresh",
        )

    async def callback(self, interaction: discord.Interaction) -> None:
        view = self.view
        if isinstance(view, RecruitPanelView):
            await view.refresh(interaction)


class RecruitPanelView(discord.ui.LayoutView):
    """채널에 고정되는 파티 모집 홈 화면."""

    def __init__(self, bot: commands.Bot, store: RecruitStore, guild_id: int | None = None) -> None:
        super().__init__(timeout=None)
        self.bot = bot
        self.store = store
        self.guild_id = guild_id
        self._build()

    def _build(self) -> None:
        active = self.store.open_for_guild(self.guild_id) if self.guild_id else []
        open_seats = sum(
            max(0, int(state.get("max_members", 5)) - len(state.get("member_ids", [])))
            for state in active
        )

        container = discord.ui.Container(accent_color=ACCENT_COLOR)
        container.add_item(
            discord.ui.TextDisplay(
                "# 🎮 PARTY FINDER\n"
                "오버워치 파티를 만들고, 참가하고, 내 파티를 한곳에서 관리하세요."
            )
        )
        container.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.small))
        container.add_item(
            discord.ui.TextDisplay(
                f"**🟢 모집 중**  `{len(active)}개`　"
                f"**👥 남은 자리**  `{open_seats}석`　"
                "**⏱️ 자동 정리**  `90분`"
            )
        )
        container.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.large))

        if active:
            container.add_item(discord.ui.TextDisplay("## 🔥 지금 참여할 수 있는 파티"))
            for state in active[:8]:
                container.add_item(
                    discord.ui.Section(
                        compact_party_text(state),
                        accessory=discord.ui.Button(
                            label="열기",
                            emoji="↗️",
                            style=discord.ButtonStyle.link,
                            url=party_link(state),
                        ),
                    )
                )
            if len(active) > 8:
                container.add_item(
                    discord.ui.TextDisplay(
                        f"-# 최근 모집 8개만 표시 중 · 표시되지 않은 모집 {len(active) - 8}개"
                    )
                )
        else:
            container.add_item(
                discord.ui.TextDisplay(
                    "## 💤 대기 중인 파티가 없어요\n"
                    "첫 모집을 열어보세요. 모드와 티어를 고르면 바로 등록됩니다."
                )
            )

        container.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.large))
        container.add_item(
            discord.ui.ActionRow(
                _PanelCreateButton(),
                _PanelMyPartyButton(),
                _PanelRefreshButton(),
            )
        )
        container.add_item(
            discord.ui.TextDisplay(
                "-# 한 사람은 한 파티에만 참가할 수 있어요 · 상세 관리 화면은 본인에게만 표시됩니다"
            )
        )
        self.add_item(container)

    async def open_builder(self, interaction: discord.Interaction) -> None:
        if interaction.guild is None:
            await interaction.response.send_message("서버 안에서만 사용할 수 있습니다.", ephemeral=True)
            return

        existing = self.store.find_user_recruit(interaction.guild.id, interaction.user.id)
        if existing:
            from .manage import RecruitDashboardView

            await interaction.response.send_message(
                view=RecruitDashboardView(
                    self.bot,
                    self.store,
                    interaction.guild.id,
                    interaction.user.id,
                ),
                ephemeral=True,
            )
            return

        from .builder import RecruitBuilderView

        builder = RecruitBuilderView(self.bot, self.store, interaction)
        await interaction.response.send_message(view=builder, ephemeral=True)

    async def open_dashboard(self, interaction: discord.Interaction) -> None:
        if interaction.guild is None:
            await interaction.response.send_message("서버 안에서만 사용할 수 있습니다.", ephemeral=True)
            return
        from .manage import RecruitDashboardView

        await interaction.response.send_message(
            view=RecruitDashboardView(
                self.bot,
                self.store,
                interaction.guild.id,
                interaction.user.id,
            ),
            ephemeral=True,
        )

    async def refresh(self, interaction: discord.Interaction) -> None:
        if interaction.guild is None:
            return
        await interaction.response.defer(ephemeral=True)
        await refresh_panel(self.bot, self.store, interaction.guild.id)
        from .manage import RecruitNoticeView

        await interaction.followup.send(
            view=RecruitNoticeView(
                "목록을 새로고침했어요",
                "현재 모집 상태를 다시 불러왔습니다.",
                kind="success",
            ),
            ephemeral=True,
        )


class _PostJoinButton(discord.ui.Button):
    def __init__(self, disabled: bool) -> None:
        super().__init__(
            label="참가하기",
            emoji="✅",
            style=discord.ButtonStyle.success,
            custom_id="recruit:post:join",
            disabled=disabled,
        )

    async def callback(self, interaction: discord.Interaction) -> None:
        view = self.view
        if isinstance(view, RecruitPostView):
            await view.join(interaction)


class _PostLeaveButton(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            label="참가 취소",
            emoji="↩️",
            style=discord.ButtonStyle.secondary,
            custom_id="recruit:post:leave",
        )

    async def callback(self, interaction: discord.Interaction) -> None:
        view = self.view
        if isinstance(view, RecruitPostView):
            await view.open_leave_confirm(interaction)


class _PostManageButton(discord.ui.Button):
    def __init__(self) -> None:
        super().__init__(
            label="파티 관리",
            emoji="⚙️",
            style=discord.ButtonStyle.primary,
            custom_id="recruit:post:manage",
        )

    async def callback(self, interaction: discord.Interaction) -> None:
        view = self.view
        if isinstance(view, RecruitPostView):
            await view.open_manage(interaction)


class RecruitPostView(discord.ui.LayoutView):
    """채널에 게시되는 공개 파티 카드."""

    def __init__(self, bot: commands.Bot, store: RecruitStore, state: RecruitState) -> None:
        super().__init__(timeout=None)
        self.bot = bot
        self.store = store
        self.state = state
        self._build()

    def _build(self) -> None:
        count, maximum = member_count(self.state)
        status_icon, status_text, accent_color = status_info(self.state)
        note = str(self.state.get("note") or "편하게 참가하고 함께 플레이해요.")
        voice_channel_id = self.state.get("voice_channel_id")

        container = discord.ui.Container(accent_color=accent_color)
        container.add_item(
            discord.ui.TextDisplay(
                f"# {mode_emoji(str(self.state['mode']))} {self.state['mode']}\n"
                f"> {note}"
            )
        )
        container.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.small))
        container.add_item(
            discord.ui.TextDisplay(
                f"**{status_icon} {status_text}**　`{count}/{maximum}명`\n"
                f"`{progress_bar(count, maximum)}`"
            )
        )
        container.add_item(
            discord.ui.TextDisplay(
                f"**🏆 티어**　{self.state['tier']}\n"
                f"**{role_emoji(str(self.state['role']))} 필요한 역할**　{self.state['role']}\n"
                f"**👑 파티장**　<@{self.state['host_id']}>\n"
                f"**🔊 음성방**　{f'<#{voice_channel_id}>' if voice_channel_id else '아직 연결되지 않음'}"
            )
        )
        container.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.small))
        container.add_item(discord.ui.TextDisplay(f"## 👥 참가자\n{member_mentions(self.state)}"))
        container.add_item(
            discord.ui.TextDisplay(
                f"-# 생성 <t:{self.state['created_at']}:R> · 자동 만료 <t:{self.state['expires_at']}:R>"
            )
        )

        closed = bool(self.state.get("closed", False))
        full = count >= maximum
        buttons: list[discord.ui.Button] = [
            _PostJoinButton(disabled=closed or full),
            _PostLeaveButton(),
            _PostManageButton(),
        ]
        if voice_channel_id:
            buttons.append(
                discord.ui.Button(
                    label="음성방 이동",
                    emoji="🔊",
                    style=discord.ButtonStyle.link,
                    url=f"https://discord.com/channels/{self.state['guild_id']}/{voice_channel_id}",
                )
            )

        container.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.large))
        container.add_item(discord.ui.ActionRow(*buttons))
        container.add_item(
            discord.ui.TextDisplay(
                "-# 참가와 취소는 즉시 반영됩니다 · 수정, 연장, 마감, 삭제는 파티 관리에서 처리합니다"
            )
        )
        self.add_item(container)

    def _can_manage(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id == int(self.state["host_id"]):
            return True
        return isinstance(interaction.user, discord.Member) and interaction.user.guild_permissions.manage_messages

    async def _edit_public(self) -> None:
        await refresh_recruit_post(self.bot, self.store, self.state)
        await refresh_panel(self.bot, self.store, int(self.state["guild_id"]))

    async def join(self, interaction: discord.Interaction) -> None:
        if interaction.guild is None:
            return
        if not self.store.is_open(self.state):
            await interaction.response.send_message("이미 마감되었거나 만료된 모집입니다.", ephemeral=True)
            return
        if interaction.user.id in self.state["member_ids"]:
            from .manage import RecruitDashboardView

            await interaction.response.send_message(
                view=RecruitDashboardView(
                    self.bot,
                    self.store,
                    interaction.guild.id,
                    interaction.user.id,
                ),
                ephemeral=True,
            )
            return

        other = self.store.find_user_recruit(interaction.guild.id, interaction.user.id)
        if other and int(other["message_id"]) != int(self.state["message_id"]):
            await interaction.response.send_message("다른 모집에 참가 중입니다.", ephemeral=True)
            return
        if len(self.state["member_ids"]) >= int(self.state["max_members"]):
            await interaction.response.send_message("파티 정원이 가득 찼습니다.", ephemeral=True)
            return

        self.state["member_ids"].append(interaction.user.id)
        if len(self.state["member_ids"]) >= int(self.state["max_members"]):
            self.state["closed"] = True
            self.state["closed_reason"] = "정원 마감"
        self.store.save()

        await interaction.response.defer(ephemeral=True)
        await self._edit_public()
        from .manage import RecruitDashboardView

        await interaction.followup.send(
            view=RecruitDashboardView(
                self.bot,
                self.store,
                interaction.guild.id,
                interaction.user.id,
            ),
            ephemeral=True,
        )

        if self.state.get("closed_reason") == "정원 마감" and interaction.channel:
            try:
                await interaction.channel.send(
                    f"🎉 <@{self.state['host_id']}> 파티 정원이 모두 찼습니다.",
                    reference=interaction.message,
                    mention_author=False,
                    delete_after=30,
                )
            except discord.HTTPException:
                pass

    async def open_leave_confirm(self, interaction: discord.Interaction) -> None:
        user_id = interaction.user.id
        if user_id == int(self.state["host_id"]):
            await interaction.response.send_message(
                "파티장은 참가 취소 대신 파티 관리에서 모집을 삭제해 주세요.",
                ephemeral=True,
            )
            return
        if user_id not in self.state.get("member_ids", []):
            await interaction.response.send_message("이 파티에 참가 중이 아닙니다.", ephemeral=True)
            return

        from .manage import RecruitLeaveConfirmView

        await interaction.response.send_message(
            view=RecruitLeaveConfirmView(
                self.bot,
                self.store,
                self.state,
                interaction.user.id,
            ),
            ephemeral=True,
        )

    async def open_manage(self, interaction: discord.Interaction) -> None:
        if not self._can_manage(interaction):
            from .manage import RecruitDashboardView

            if interaction.guild is None:
                return
            await interaction.response.send_message(
                view=RecruitDashboardView(
                    self.bot,
                    self.store,
                    interaction.guild.id,
                    interaction.user.id,
                ),
                ephemeral=True,
            )
            return

        from .manage import RecruitManageView

        await interaction.response.send_message(
            view=RecruitManageView(
                self.bot,
                self.store,
                self.state,
                interaction.user.id,
            ),
            ephemeral=True,
        )
